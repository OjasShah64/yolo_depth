...
# ROS service imports
from yolo_depth_node1.srv import GetClosestObject, GetClosestObjectResponse
from yolo_depth_node1.srv import GetBiggestObject, GetBiggestObjectResponse
from yolo_depth_node1.srv import CountObjects, CountObjectsResponse
...
class YoloDepthCalibrator:
    def __init__(self):
        ...
        # Results from latest processed frame
        self.last_detections = []
        self.last_calibrated_depth_map = None
        ...
        # Service definitions
        rospy.Service("/get_closest_object", GetClosestObject, self.handle_get_closest_object)
        rospy.Service("/get_biggest_object", GetBiggestObject, self.handle_get_biggest_object)
        rospy.Service("/count_objects", CountObjects, self.handle_count_objects)
    ...
    def process_frame(self):
        ...
        for i, det in enumerate(detections):
            ...
            if 0.1 < sensor_depth < 10.0 and da_depth > 0:
                ...
                calibrated_depth_map = da_depth_map * scale
        ...
        self.last_calibrated_depth_map = calibrated_depth_map
    ...
    def run_depth_anything(self, bgr_np):
        ...
        return 1.0 - normalized  # inverted: closer = smaller

    # -------------------------------
    # ROS Service Callbacks
    # -------------------------------

    def handle_get_closest_object(self, req):
        if not self.last_detections:
            return GetClosestObjectResponse(depth=-1.0, x=-1, y=-1)
        closest = min(self.last_detections, key=lambda d: d['depth'])
        x1, y1, x2, y2 = closest['bbox']
        cx, cy = (x1 + x2) // 2, (y1 + y2) // 2
        return GetClosestObjectResponse(depth=closest['depth'], x=cx, y=cy)

    def handle_get_biggest_object(self, req):
        if not self.last_detections:
            return GetBiggestObjectResponse(depth=-1.0, area=-1.0)
        biggest = max(self.last_detections, key=lambda d: d['area'])
        return GetBiggestObjectResponse(depth=biggest['depth'], area=biggest['area'])

    def handle_count_objects(self, req):
        return CountObjectsResponse(count=len(self.last_detections))
